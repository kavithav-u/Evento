...
const DetailsPage = () => {
  ...
  const handleCheckDates = async () => {
    try {
      const isDateBooked = await checkDateAvailability(
        startDate,
        endDate,
        bookings
      );
      if (isDateBooked === true) {
        toast.warning("Already booked. Please select another date or hall.");
      } else {
  ...
  const SubmitPayment = async (totalAmount) => {
    if (!userInfo) {
      // If userInfo is not available, redirect to the sign-in page
      toast.warning("Please log first.");
      return;
    } else {
              ...
              {halls.map((hall) => (
                <div key={hall?._id} className="mb-6">
                  ...
                  <p className="flex items-center mt-6 gap-2 text-slate-600 text-sm">
                    ...
                    {hall?.parking && "Parking"}
                    {hall?.catering && (
                      <FaPlateWheat className="text-green-700" />
                    )}
                    {hall?.catering && "Catering"}
                  </p>
                  <p className="flex items-center mt-6 gap-2 text-slate-600 text-sm"></p>
            ...
            <div className="hotelDetails bg-emerald-100 rounded-2xl">
              <div className="hotelDetailsPrice">
                ...
                {totalAmount ? (
                  ...
                  <h3>
                    <b className="font-poppins p-20">
                      $ {halls[0]?.pricePerDay} per{" "}
                      {totalDays ? `${totalDays} days` : "1 day"}
                    </b>
                  </h3>
                )}
