...
import { useUpdateAboutMutation, useFetchAboutMutation, useDeleteAboutMutation } from '../../Slices/adminApiSlice';
...
const AboutList = () => {
  ...
  return (
    <div>
    <div>
<Link to="/admin/about/new" className="bg-blue-500 text-white px-4 py-2 rounded-md">
Add New
</Link>
 ...
 aboutData.map((about) => (
   <tr key={about?._id}>
     ...
     <td>
       ...
       <td>
          <button className='text-green-600' onClick={() => handleEdit(about)}><FaEdit /></button>
          <button className='ml-3 text-red-700' onClick= {() => handleDelete(about)}><FaTrash /></button>
        </td>
   </tr>
 ))
) : (
