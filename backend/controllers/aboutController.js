...
const newAbout = async(req,res) => {
    try {
        ...
        if(existing) {
            ...
            res.status(200).json({success:true, message:"New About Added", About:newAbout})
        }
    } catch(error) {
        res.status(500).json({message:"Internal Server Error"})
    }
...
const updateAbout = async (req,res) => {
    try {
        ...
        if(!existingAbout) {
            ...
            res.status (200) .json({succss:true, about:updatedAbout, message:"About updated successfully"});
        }
    } catch (error) {
        res.status(500).json({ message: "Internal Server Error" });
      }
